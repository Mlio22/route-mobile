...
import { Manuallocationprops } from '../types/App';
...
const Manuallocation = (props:Manuallocationprops) => {
  ...
  const [coordinates, setCoordinates] = useState<number[]>([107.6351264, -6.972025]);
  ...
  return (
      <LocationDetailsContextProvider>
        <View style={{ flex: 1 }}>
          ...
          <MapboxGL.MapView style={{ flex: 1 }} ref={mapview} onTouchMove={async(e) => {
            try{
            ...
            <View style={styles.containerstatus}>
              <View style={styles.StatusLokasiDanTujuan}>
                <View style={styles.Framedetailmanualalocation}>
                  ...
                  />
                  <View>
                    <Text style={styles.pinpointtext}>Latitude: {coordinates[1].toFixed(5)}</Text>
                    <Text style={styles.pinpointtext}>Longitude: {coordinates[0].toFixed(5)}</Text>
                  </View>
